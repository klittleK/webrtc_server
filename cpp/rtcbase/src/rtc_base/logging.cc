...
const char* FilenameFromPath(const char* file) {
  ...
  if (LogMessage::GetLogPathPrefix() != "") {
    end1 = ::strstr(file, LogMessage::GetLogPathPrefix().c_str());
  }
...
std::string LogMessage::log_path_prefix_ = "";

static const char* GetSeverityStr(LoggingSeverity sev) {
  switch (sev) {
    case LS_VERBOSE:
        return "VERBOSE";
    case LS_INFO:
...
LogMessage::LogMessage(const char* file,
  ...
  print_stream_ << GetSeverityStr(sev) << ": "; 
  ...
  // 年
  print_stream_ << now_tm.tm_year + 1900 << "-";
  
  // 月
  if (now_tm.tm_mon + 1 <= 9) {
      print_stream_ << "0";
  }  
  print_stream_ << now_tm.tm_mon + 1 << "-";
  
  // 日
  if (now_tm.tm_mday <= 9) {
      print_stream_ << "0";
  }
  print_stream_ << now_tm.tm_mday << " ";
 
  // 时
  if (now_tm.tm_hour <= 9) {
      print_stream_ << "0";
  }
  print_stream_ << now_tm.tm_hour << ":";
  
  // 分
  if (now_tm.tm_min <= 9) {
      print_stream_ << "0";
  }
  print_stream_ << now_tm.tm_min << ":";
  
  // 秒
  if (now_tm.tm_sec <= 9) {
      print_stream_ << "0";
  }
  print_stream_ << now_tm.tm_sec << " ";
  */
...
std::string LogMessage::GetLogPathPrefix() {
  return log_path_prefix_;
}

void LogMessage::SetLogPathPrefix(const std::string& prefix) {
  log_path_prefix_ = prefix;
}
