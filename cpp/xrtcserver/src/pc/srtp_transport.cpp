...
bool SrtpTransport::set_rtp_params(int send_cs, const uint8_t *send_key, size_t send_key_len, const std::vector<int> &send_extension_ids, int recv_cs, const uint8_t *recv_key, size_t recv_key_len, const std::vector<int> &recv_extension_ids)
{
    ...
    RTC_LOG(LS_INFO) << "SRTP " << (new_session ? "activated" : "updated") << " params: send_crypto suit " << send_cs
    << " recv crypto suit " << recv_cs;
...
void SrtpTransport::get_send_auth_tag_len(int* rtp_auth_tag_len, int* rtcp_auto_tag_len) {
    if (_send_session) {
        _send_session->get_auth_tag_len(rtp_auth_tag_len, rtcp_auto_tag_len);
    }
}
