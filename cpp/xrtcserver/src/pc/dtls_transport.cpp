...
const size_t k_dtls_record_header_len = 13; // DTLS record header length
const size_t k_max_dtls_packet_len = 2048;
const size_t k_max_pending_packets = 2;
const size_t k_min_rtp_packet_len = 12; // RTP packet minimum length
...
bool is_dtls_client_hello_packet(const char* buf, size_t len) {
    // 检查数据包是否为DTLS ClientHello包
    if (!is_dtls_packet(buf, len)) {
        return false;
    }

    const uint8_t* u = reinterpret_cast<const uint8_t*>(buf);
    // DTLS ClientHello的记录类型为22
    return  len > k_dtls_record_header_len + 4 && (u[0] == 22 && u[13] == 1); // 至少包含记录头和ClientHello的最小长度
}
...
bool xrtc::StreamInterfaceChannel::on_received_packet(const char *data, size_t size) {
    if (_packets.size() > 0) {
    ...
    switch (_dtls_state) {
        case DtlsTransportState::k_new:
            ...
            if (_dtls) {
                ...
                RTC_LOG(LS_WARNING) << to_string() << ": Received packet before we know if we are doing DTLS or not";
            }
...
bool DtlsTransport::set_remote_fingerprint(const std::string& digest_alg, const uint8_t* digest, size_t digest_len) {
    ...
    // ClientHello packet 先到，answer sdp 后到
...
bool DtlsTransport::_setup_dtls() {
    ...
    if (_remote_fingerprint_value.size() != 0 && !_dtls->SetPeerCertificateDigest(
        _remote_fingerprint_alg, _remote_fingerprint_value.data(), _remote_fingerprint_value.size() 
    )) {
