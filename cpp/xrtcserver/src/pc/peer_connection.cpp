...
static int parse_msid_info(MediaContentDescription* content, std::vector<SsrcInfo>& ssrc_info, const std::string& line) {
    // 处理msid
    if (line.find("a=msid:") != std::string::npos) {
        // a=msid:<stream-id> <track-id>
        std::vector<std::string> fields;
        rtc::tokenize(line.substr(7), ' ', &fields);
        if (fields.size() != 2) {
            RTC_LOG(LS_WARNING) << "parse a=msid error: " << line;
            return -1;
        }

        content->set_stream_id(fields[0]);
        content->set_track_id(fields[1]);

        for (auto& ssrc : ssrc_info) {
            ssrc.stream_id = fields[0];
            ssrc.track_id = fields[1];
        }
...
static int parse_ssrc_info(MediaContentDescription* content, std::vector<SsrcInfo>& ssrc_info, const std::string& line) {
    ...
    // msid
    if (content) {
        if (!content->stream_id().empty()) {
            iter->stream_id = content->stream_id();
        }
        if (!content->track_id().empty()) {
            iter->track_id = content->track_id();
        }
    }
...
static int parse_ssrc_group_info(std::vector<SsrcGroup>& ssrc_groups, const std::string& line) {
    ...
    // a=ssrc-group:<semantics> <ssrc-id> <ssrc-id> ...
    std::vector<std::string> fields;
    rtc::split(line.substr(2), ' ', &fields);
    if (fields.size() < 2) {
        RTC_LOG(LS_WARNING) << "parse a=ssrc-group error: " << line;
        return -1;
    }
    ...
    for (size_t i = 1; i < fields.size(); ++i) {
        ...
        if (!rtc::FromString(fields[i], &ssrc_id)) {
            RTC_LOG(LS_WARNING) << "invalid ssrc_id in ssrc-group, line: " << line;
            return -1;
        }
...
static void create_track_from_ssrc_info(const std::vector<SsrcInfo>& ssrc_infos, std::vector<StreamParams>& tracks) {
...
int PeerConnection::set_remote_sdp(const std::string &sdp) {
    ...
    for (auto field : fields) {
        ...
        if ("audio" == media_type) {
            ...
            if (parse_msid_info(audio_content.get(), audio_ssrc_info, field) != 0) {
                return -1;
            }

            if (parse_ssrc_info(audio_content.get(), audio_ssrc_info, field) != 0) {
                return -1;
            }
        }
        else if ("video" == media_type) {
            ...
            if (parse_msid_info(video_content.get(), video_ssrc_info, field) != 0) {
                return -1;
            }
            ...
            if (parse_ssrc_info(video_content.get(), video_ssrc_info, field) != 0) {
                return -1;
            }
        }
    }
