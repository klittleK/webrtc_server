...
int DtlsSrtpTransport::send_rtp(const char* buf, size_t size) {
    ...
    int len = packet.size();
    uint16_t seq_num = parase_rtp_sequence_number(packet);
...
void DtlsSrtpTransport::_setup_dtls_srtp() {
    ...
    if (!_extract_params(_rtp_dtls_transport, &selected_crypto_suite, &send_key, &recv_key) || 
        !set_rtp_params(selected_crypto_suite, &send_key[0], send_key.size(), send_extension_ids, selected_crypto_suite, &recv_key[0], recv_key.size(), recv_extension_ids)) {
...
bool DtlsSrtpTransport::_extract_params(DtlsTransport* dtls_transport,
    ...
    if (!dtls_transport->export_keying_material(k_dtls_srtp_exporter_label, NULL, 0, false, &dtls_buffer[0], dtls_buffer.size())) {
