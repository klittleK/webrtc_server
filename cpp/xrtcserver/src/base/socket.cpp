...
int create_tcp_server(const char* addr, int port) {
    ...
    // 2、设置端口复用（服务器重启后客户端可能还连着，不设置复用服务端就bind不上端口了）
    int on = 1;
...
int sock_setnodelay(int sock) {
    ...
    if(-1 == ret) {
        RTC_LOG(LS_WARNING) << "set nodelay failed: " << strerror(errno) << ", errno: " << errno << ", fd: " << sock;
        return -1;
    }
...
int sock_bind(int sock, sockaddr *addr, socklen_t len, int min_port, int max_port) {
    int ret = -1;
    if (0 == min_port && 0 == max_port) {   // 让os自动选端口
...
int sock_recv_from(int sock, char *buf, size_t size, sockaddr *addr, socklen_t addr_len) {
...
int sock_send_to(int sock, const char *buf, size_t len, int flag, sockaddr *addr, socklen_t addr_len) {
