...
bool SignalingWorker::start() {
    ...
    _thread = new std::thread([=]() {
        RTC_LOG(LS_INFO) << "signaling worker event loop start, worker id: " << _worker_id;
        _el->start();
        RTC_LOG(LS_INFO) << "signaling worker event loop stop, worker id: " << _worker_id;
    });
...
// 连接进来的话是有内容的，所以需要既通知NEW_CONN，又把内容放进队列
...
void SignalingWorker::_write_reply(int fd) {
    ...
    while(!c->reply_list.empty()) {
        ...
        if (-1 == nwritten) {
            ...
            RTC_LOG(LS_WARNING) << "write 0 bytes, fd: " << c->fd << ", worker id: " << _worker_id;
...
void SignalingWorker::_new_conn(int fd) {
    ...
    _el->start_timer(c->timer_watcher, 500000); // 500ms
...
void SignalingWorker::_response_server_offer(std::shared_ptr<RtcMsg> msg) {
    ...
    if(!c) {    // 这里判空是因为请求处理完后，链接可能已经超时关闭了
        return;
    }
    ...
    rtc::Slice reply(buf, XHEAD_SIZE + res_xh->body_len);
    // 到这里响应包就写完了，接着写入tcp链接（异步链接，事件触发方式写入）
    _add_reply(c, reply);
}
...
void SignalingWorker::_process_notify(int msg) {
    ...
    case RTC_MSG:
        ...
        break;
    default:
        RTC_LOG(LS_WARNING) << "unknow msg: " << msg;
        break;
    }
}
