...
int RtcStreamManager::create_pull_stream(uint64_t uid, const std::string &stream_name, 
    ...
    if(!push_stream) {
        RTC_LOG(LS_WARNING) << "Stream not found, uid: " << uid << ", stream_name: " << stream_name << ", log_id: " << log_id;
        return -1;
    }
    ...
    PullStream* stream = new PullStream(_el, _allocator.get(), uid, stream_name, audio, video, log_id);
    stream->register_listener(this);
...
int RtcStreamManager::set_answer(uint64_t uid, const std::string &stream_name, const std::string &answer, const std::string &stream_type, uint32_t log_id) {
    RTC_LOG(LS_WARNING) << stream_type;
    if ("push" == stream_type) {
        PushStream* push_stream = _find_push_stream(stream_name);
        if (!push_stream) {
            RTC_LOG(LS_WARNING) << "push stream not found, uid: " << uid << ", stream_name: " << stream_name
