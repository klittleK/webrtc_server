...
bool StunMessage::_validate_message_integrity_of_type(uint16_t mi_attr_type, size_t mi_attr_size, const char* data, size_t size, const std::string& password) {
    ...
    size_t ret = rtc::ComputeHmac(rtc::DIGEST_SHA_1, password.c_str(), password.length(),
                                  temp_data.get(),current_pos, hmac, sizeof(hmac));
    if (ret != k_stun_message_integrity_size) {
        return false;
    }
    return memcmp(data + mi_pos + k_stun_attribute_header_size, hmac, k_stun_message_integrity_size) == 0;
}

bool StunMessage::add_message_integrity(const std::string &password) {
    return _add_message_integrity_of_type(
        STUN_ATTR_MESSAGE_INTEGRITY,
        k_stun_message_integrity_size,
...
bool StunMessage::read(rtc::ByteBufferReader *buf)
{
    ...
    if (rtc::NetworkToHost32(magic_cookie_int) != k_stun_magic_cookie) { // 老版本
...
bool StunAddressAttribute::write(rtc::ByteBufferWriter* buf) {
    ...
    if (STUN_ADDRESS_UDEF == stun_family) {
        RTC_LOG(LS_WARNING) << "StunAddressAttribute::write failed, because address family is undefined";
        return false;
    }
...
rtc::IPAddress StunXorAddressAttribute::_get_xored_ip(){
    ...
    return rtc::IPAddress(); // 返回一个空的IP地址
}
