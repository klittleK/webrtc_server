...
UDPPort::UDPPort(EventLoop *el, const std::string &transport_name, IceCandidateComponent component, IceParameters ice_params) :
    _el(el),
...
std::string compute_foundaton(const std::string& type, 
...
int UDPPort::create_ice_candidate(Network *network, int min_port, int max_port, Candidate &c) {
    ...
    int port = 0;
    if (sock_get_address(_socket, nullptr, &port) != 0) {   // 获取ip和端口的方法
        return -1;
    }
    ...
    c.foundation = compute_foundaton(c.type, c.protocol, "", c.address);
...
IceConnection *xrtc::UDPPort::create_connection(const Candidate &remote_candidate) {
    ...
    if (!ret.second && ret.first->second != conn) {
        RTC_LOG(LS_WARNING) << to_string() << ": create ice connection on an existing remote address, addr: "
...
bool UDPPort::get_stun_message(const char *data, size_t len, const rtc::SocketAddress& addr, std::unique_ptr<StunMessage> *out_msg, std::string* out_username) {
