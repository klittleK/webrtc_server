...
const int RTT_RATIO = 3; // 老rtt占0.75权重
const int MIN_RTT = 100; // 最小RTT值
const int MAX_RTT = 60000; // 最大RTT值

ConnectionRequest::ConnectionRequest(IceConnection *conn):
    StunRequest(new StunMessage()),
    _connection(conn)
{
...
void IceConnection::_on_stun_send_packet(StunRequest *request, const char *buf, size_t len) {
    ...
    if (ret < 0) {
        RTC_LOG(LS_WARNING) << to_string() << ": Failed to send STUN binding request: ret=" << ret << ", id=" << rtc::hex_encode(request->id());
    }
}
...
void IceConnection::received_ping_response(int rtt) {
    ...
    set_state(IceCandidatePairState::SUCCESSED);
}
...
void IceConnection::update_state(int64_t now) {
    ...
    if (_write_state == STATE_WRITABLE 
            && _too_many_pings_fails(CONNECTION_WRITE_CONNECT_FAILS, rtt, now) 
            && _too_long_without_response(CONNECTION_WRITE_CONNECT_TIMEOUT, now)) {
        RTC_LOG(LS_WARNING) << to_string() << ": Unwritable after " << CONNECTION_WRITE_CONNECT_FAILS 
            << " pings fails and " << now - _pings_since_last_response[0].sent_time << " ms without response";
    ...
    if ((_write_state == STATE_WRITE_UNRELIABLE || _write_state == STATE_WRITE_INIT) 
            && _too_long_without_response(CONNECTION_WRITE_TIMEOUT, now)) {
                RTC_LOG(LS_WARNING) << to_string() << ": Timeout after " <<  now - _pings_since_last_response[0].sent_time << " ms without response";
...
void IceConnection::send_stun_binding_response(StunMessage *stun_msg) {
    ...
    if (!username_attr) {
        RTC_LOG(LS_WARNING) << "send stun binding response failed, because username attribute not found";
        return;
    }
        ...
        switch (stun_msg->type()) {
            case STUN_BINDING_REQUEST: {
                if (remote_ufrag != remote.username) {
                    ...
                    _port->send_binding_error_response(stun_msg.get(), remote.address, STUN_ERROR_UNAUTHORIZED, STUN_ERROR_REASON_UNAUTHORIZED);
                } else {
...
void xrtc::IceConnection::maybe_set_remote_ice_params(const IceParameters &ice_params) {
